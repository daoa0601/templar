...
export async function initializeTelecomIncidentWorkspace(options: {
  ...
  const incidentsRoot = path.join(path.resolve(options.templarHome), "incidents");
  await mkdir(incidentsRoot, { recursive: true, mode: 0o700 });
  const root = path.join(incidentsRoot, options.runId);
  try {
  ...
  try {
    ...
    await git(root, ["commit", "-m", "Initialize immutable telecom incident evidence"]);
  } catch (cause) {
    throw new TemplarError({
      code: "INTERNAL_ERROR",
